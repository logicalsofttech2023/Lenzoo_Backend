    ...
    try:
        ...
        with mp_face_mesh.FaceMesh(static_image_mode=True, max_num_faces=1, refine_landmarks=True) as face_mesh:
            ...
            if len(landmarks) < 478:
                print("❌ Incomplete facial landmarks detected. Expected at least 478 for detailed iris detection.")
            ...
            if jaw_width_pixel > 0: # Avoid division by zero
                pixels_per_mm = jaw_width_pixel / 155.0 # This is a very rough estimate!
            else:
            ...
            if (abs(ratios['length_to_width'] - 1) <= 0.2 and # length approx equal to width
                abs(ratios['forehead_to_cheekbone'] - 1) <= TIGHT_SIMILARITY and # forehead, cheekbone, jaw approx equal
                abs(ratios['jaw_to_cheekbone'] - 1) <= TIGHT_SIMILARITY):
                face_shape = "Square"

            # 2. Diamond - prominent cheekbones, narrower forehead and jaw
            elif (ratios['jaw_to_cheekbone'] < (1 - SIGNIFICANT_DIFFERENCE) and
                  ratios['forehead_to_cheekbone'] < (1 - SIGNIFICANT_DIFFERENCE) and
                  ratios['length_to_width'] > (1 + MODERATE_SIMILARITY)): # Longer than wide
                face_shape = "Diamond"

            # 3. Heart - wider forehead, narrower jaw/chin
            elif (ratios['forehead_to_cheekbone'] > (1 + MODERATE_SIMILARITY) and # Forehead significantly wider than cheekbones
                  ratios['forehead_to_jaw'] > (1 + SIGNIFICANT_DIFFERENCE)): # Forehead significantly wider than jaw
                face_shape = "Heart"

            # 4. Triangle (Pear) - wider jaw, narrower forehead
            elif (ratios['jaw_to_cheekbone'] > (1 + MODERATE_SIMILARITY) and # Jaw significantly wider than cheekbones
                  ratios['forehead_to_jaw'] < (1 - SIGNIFICATE_DIFFERENCE)): # Forehead significantly narrower than jaw
                face_shape = "Triangle"

            # 5. Round - length approx equal to width, soft angles (all widths similar)
            elif (abs(ratios['length_to_width'] - 1) <= MODERATE_SIMILARITY and
                  abs(ratios['forehead_to_cheekbone'] - 1) <= MODERATE_SIMILARITY and
                  abs(ratios['jaw_to_cheekbone'] - 1) <= MODERATE_SIMILARITY):
                face_shape = "Round"

            # 6. Rectangle (Oblong) - long face, similar widths
            elif (ratios['length_to_width'] > (1 + SIGNIFICANT_DIFFERENCE) and # Significantly longer than wide
                  abs(ratios['forehead_to_cheekbone'] - 1) <= MODERATE_SIMILARITY and # All widths similar
                  abs(ratios['jaw_to_cheekbone'] - 1) <= MODERATE_SIMILARITY):
            ...
            landmark_accuracy = (detected_landmarks / 478) * 100 # Assuming 478 is full set including iris
            ...
            print(f"✅ Face processed. PD: {pd_mm:.1f} mm, PH: {pupil_height_mm:.1f} mm, Shape: {face_shape}, Accuracy: {final_accuracy}%")
